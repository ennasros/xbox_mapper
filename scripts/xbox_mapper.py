...
LINEAR_SCALE = rospy.get_param('~linear_scale', 1)
ANGULAR_SCALE = rospy.get_param('~angular_scale', 1)
MIN_TOGGLE_DUR = 0.5  #
...
def joy_cb(Joy):
    ...
    cmd.twist.linear.x = drive_cmd * LINEAR_SCALE
    cmd.twist.angular.y = flipper_cmd
    cmd.twist.angular.z = turn_cmd * ANGULAR_SCALE
    pub.publish(cmd)
    seq += 1
